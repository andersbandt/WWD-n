//*****************************************************************************
//!
//! @file inv_imu_driver.c
//! @author Anders Bandt
//! @brief Contains IMU functions for interfacing with the 'inv_imu_device' struct
//! @version 0.9
//! @date November 2023
//!
//*****************************************************************************
                 ...
                 void (*sensor_event_cb)(inv_imu_sensor_event_t *event), Display_Handle display)
{
	...
	Display_printf(display, 0, 0, "... status1: %d\r\n", status);
    
	/* Reset device */
	Display_printf(display, 0, 0, "... issuing device reset");
	status |= inv_imu_device_reset(s);
    Display_printf(display, 0, 0, "... status2: %d\r\n", status);

	/* Init transport layer */
	status |= inv_imu_init_transport(s);
    Display_printf(display, 0, 0, "... status3: %d\r\n", status);
    ...
    /* Display_printf(display, 0, 0, "\tPWR_MGMT0[0x%x] = 0x%x", PWR_MGMT0, data);     */
    /* uint16_t regaddr = 0x01; */
    /* int mystatus = inv_imu_read_reg(s, regaddr, 1, &data); */
    /* Display_printf(display, 0, 0, "DEBUG: register read got reg/data [%x][%x] with status: %d", regaddr, data, mystatus); */
    // END DEBUG

    
	/* Read and set endianness for further processing */
	status |= inv_imu_get_endianness(s);
    Display_printf(display, 0, 0, "... status4: %d\r\n", status);

	/* Initialize hardware */
	Display_printf(display, 0, 0, "... initializing hardware");
	status |= init_hardware_from_ui(s);
    Display_printf(display, 0, 0, "... status5: %d\r\n", status);


	Display_printf(display, 0, 0, "... finished with sub function IMU config.");
...
/*
 * inv_imu_set_config_int1L reads from INT_SOURCE0 and INT_SOURCE6
 */
...
int16_t inv_imu_get_temp_register(struct inv_imu_device *s) {
    int     status = 0;
    uint8_t int_status;
    ...
    format_s16_data(s->endianness_data, &temperature[0], &temp_out);
    return temp_out;
}
...
int inv_imu_get_data_from_registers(struct inv_imu_device *s, Display_Handle display) {
	...
	if (int_status & INT_STATUS_DRDY_DATA_RDY_INT_MASK) {
		...
		if (s->sensor_event_cb) {
		    ...
		    Display_printf(display, 0, 0, "\tFUCK no sensor event callback");
		}

	}
	/*else: Data Ready was not reached*/
	else {
	    Display_printf(display, 0, 0, "\tdata not ready!\n");
	}
...
int inv_imu_get_data_from_fifo(struct inv_imu_device *s, Display_Handle display) {
	...
	/* Ensure data ready status bit is set */
	if (status |= inv_imu_read_reg(s, INT_STATUS, 1, &int_status)) {return status;}
	...
	if ((int_status & INT_STATUS_FIFO_THS_INT_MASK) || (int_status & INT_STATUS_FIFO_FULL_INT_MASK)) {
        ...
        Display_printf(display, 0, 0, "\tFIFO packet count is: %d\n", packet_count);
		while (packet_count > 0) {
			...
			// read sensor data and exit if register read failed
			if (status |= inv_imu_read_reg(s, FIFO_DATA, packet_size * packet_count, s->fifo_data)) {
				 ...
				 */
			    Display_printf(display, 0, 0, "\tFIFO read failed. Reset FIFO.\n");
				status |= inv_imu_reset_fifo(s);
				status |= inv_imu_switch_off_mclk(s);
				return status;
			}

			// this for loop decodes the frames?
			for (uint16_t i = 0; i < packet_count; i++) {
				...
				if (header->Byte == 0x80) {
                    Display_printf(display, 0, 0, "\tInvalid frame detected.");
					...
					if (header->bits.msg_bit) {
						...
						status |= inv_imu_reset_fifo(s);
						status |= inv_imu_switch_off_mclk(s);
                        Display_printf(display, 0, 0, "\tMSG bit was in FIFO header");
...
#else
					if (header->bits.twentybits_bit) // with 20-byte packets, 6B are reserved after accel value
						fifo_idx += FIFO_GYRO_DATA_SIZE; // therefore, increment `fido_idx` accordinly
#endif

                    
                    // read temperature (8 or 16 bits depending on 16B or 20B FIFO format)
...
//                       event_print(display, &event);
        ...
        Display_printf(display, 0, 0, "\tregister contents of INT_STATUS was [0x%d]", int_status);
        Display_printf(display, 0, 0, "\tReturning total_packet_count (should be 0?)");
...
int inv_imu_configure_fifo(struct inv_imu_device *s, INV_IMU_FIFO_CONFIG_t fifo_config)
{
	...
	case INV_IMU_FIFO_ENABLED:
		...
		*/
		...
		data |= (uint8_t)FIFO_CONFIG1_FIFO_MODE_STREAM | (uint8_t)FIFO_CONFIG1_FIFO_BYPASS_OFF;
        ...
        // Configure FIFO WM clear mode for INT
